...
/* display list of names from student profile table */
    ...
    public Cursor getOutpassHistory(Integer pid) {
        SQLiteDatabase db=this.getWritableDatabase();
        Cursor data = db.rawQuery("select * from "+TABLE_NAME1, null);
//        ArrayList<OutpassHistory> olist = new ArrayList<OutpassHistory>();
//
//        if(cur.getCount()!=0 ) {
//            cur.moveToFirst();
//            int c = cur.getCount();
//            for (int i = 0; i < c; i++) {
//                OutpassHistory out = new OutpassHistory();
//                String purpose = cur.getString(cur.getColumnIndex(PUR));
//                String date = cur.getString(cur.getColumnIndex(DATE));
//                Integer status = Integer.parseInt(cur.getString(cur.getColumnIndex(STATUS)));
//
